...
export class TooLostWebhookController {
  ...
  normalize(payload: any): NormalizedTooLostWebhookEvent {
    const type = String(payload.type ?? payload.eventType ?? payload.event ?? "").toUpperCase() as TooLostWebhookEventType;
    const releaseId = String(payload.releaseId ?? payload.release_id ?? payload.externalReleaseId ?? payload.tooLostReleaseId ?? "");
  ...
  private mapEventTypeToStatus(type: TooLostWebhookEventType, payload: any): DistributionStatus {
  ...
  private toDeliveryStatus(status: DistributionStatus): "PENDING" | "PROCESSING" | "PUBLISHED" | "FAILED" {
    ...
    if (status === DistributionStatus.DELIVERED || status === DistributionStatus.PUBLISHED) return "PUBLISHED";
    return "PROCESSING";
  }
}
